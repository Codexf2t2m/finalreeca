...
export default function PassengerDetailsForm({
  ...
  const departureTotal = departurePricePerSeat * departureSeats.length;
  const returnTotal = returnPricePerSeat * returnSeats.length;
  ...
  // Get boarding points
  const getBoardingPoints = (key: string) => {
    return boardingPoints[key] || [{ id: 'default', name: `${key.charAt(0).toUpperCase() + key.slice(1)} Bus Terminal` }];
  };

  const departureOriginKey = (departureBus?.routeOrigin || searchData.from || '').toLowerCase().trim();
  const departureDestinationKey = (departureBus?.routeDestination || searchData.to || '').toLowerCase().trim();
  ...
  // Only calculate return points if return trip exists
  let returnOriginPoints: BoardingPoint[] = [];
  let returnDestinationPoints: BoardingPoint[] = [];
  
  if (returnBus) {
    const returnOriginKey = (returnBus?.routeOrigin || searchData.to || '').toLowerCase().trim();
    const returnDestinationKey = (returnBus?.routeDestination || searchData.from || '').toLowerCase().trim();
              ...
              {departureBus && (
                <div className="flex justify-between border-b pb-2">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {format(new Date(departureBus.departureDate), "dd MMM yyyy")} • {departureSeats.length} seat(s)
                    </p>
                  </div>
              ...
              {returnBus && (
                <div className="flex justify-between border-b pb-2">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {format(new Date(returnBus.departureDate), "dd MMM yyyy")} • {returnSeats.length} seat(s)
                    </p>
                  </div>
