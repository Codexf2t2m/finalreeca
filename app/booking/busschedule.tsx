...
export default function BusSchedules({ searchData, onSelectBus, boardingPoints }: BusSchedulesProps) {
  ...
  const days = Array.from({ length: 7 }, (_, i) => {
    const date = searchData.departureDate ? addDays(new Date(searchData.departureDate), i) : addDays(new Date(), i);
    return {
      date,
  ...
  const filteredTrips = trips.filter((trip) => {
    const routeMatch = normalize(trip.routeOrigin) === normalize(searchData.from) && normalize(trip.routeDestination) === normalize(searchData.to);
  ...
  const filteredReturnTrips = returnTrips.filter((trip) => {
    const routeMatch = normalize(trip.routeOrigin) === normalize(searchData.to) && normalize(trip.routeDestination) === normalize(searchData.from);
    const returnDate = searchData.returnDate ? format(new Date(searchData.returnDate), "yyyy-MM-dd") : "";
    const dateMatch = format(new Date(trip.date), "yyyy-MM-dd") === returnDate;
    return routeMatch && dateMatch;
  });
        ...
        <div className="divide-y">
          ...
          {loading ? (
            ...
            filteredTrips.map((trip) => {
              ...
              const departureTime = bus.departureTime || trip.departureTime ? (bus.departureTime || format(new Date(trip.date), "HH:mm")) : format(new Date(trip.date), "HH:mm");
                  ...
                  <div className="col-span-2">
                    <div className="flex items-center space-x-3">
                      <div className="w-28 h-20 bg-gray-100 rounded-lg flex items-center justify-center relative overflow-hidden">
                        <Image src={busImg} alt={serviceType + " bus"} width={84} height={56} className="object-contain" />
                      </div>
                      <div>
        ...
        {searchData.returnDate && (
          <div className="p-6 border-t bg-gradient-to-r from-gray-50 to-gray-100">
            <label className="flex items-center gap-2 mb-4">
              <input type="checkbox" checked={showReturnTrips} onChange={() => setShowReturnTrips(!showReturnTrips)} />
